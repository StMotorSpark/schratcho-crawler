...
// Constants
const DEFAULT_SCRATCH_RADIUS = 25;
...
const DEFAULT_SCRATCHERS: Scratcher[] = [
  {
    ...
    symbol: '🪙',
    scratchRadius: DEFAULT_SCRATCH_RADIUS,
    style: {
...
// Default scratcher for fallback
const FALLBACK_SCRATCHER: Scratcher = DEFAULT_SCRATCHERS[0];

/**
 * Validates if a string is a valid emoji (entire string must be emoji characters only)
 */
function isValidEmoji(str: string): boolean {
  if (!str || str.length === 0) return false;
  // Check if the entire string consists only of emoji characters
...
/**
 * Converts an ID to a valid JavaScript constant name
 */
function toConstantName(id: string): string {
  // Replace non-alphanumeric characters with underscores
  let constName = id.replace(/[^a-zA-Z0-9]/g, '_').toUpperCase();
...
function App() {
  ...
  const handleAddScratcher = () => {
    const newId = generateUniqueId('scratcher');
  ...
  const generateScratcherCode = () => {
    ...
    // If no scratchers, provide a default
    if (scratchers.length === 0) {
      return `/**
 ...
 * Date: ${timestamp}
 * 
 * Note: No scratchers defined. Add scratchers in the tool to generate code.
 */
...
// No scratchers defined - add some in the Layout Designer
export const SCRATCHER_TYPES: Record<string, Scratcher> = {};

export function getScratcher(id: string): Scratcher {
  const defaultScratcher: Scratcher = ${JSON.stringify(FALLBACK_SCRATCHER, null, 2)};
  return SCRATCHER_TYPES[id] || defaultScratcher;
}
...
`;
    ...
    const scratcherExports = scratchers.map(s => {
      const constName = toConstantName(s.id) + '_SCRATCHER';
    ...
    // Get the first scratcher's constant name for fallback
    const firstScratcherConst = toConstantName(scratchers[0].id) + '_SCRATCHER';
...
export function getScratcher(id: string): Scratcher {
  return SCRATCHER_TYPES[id] || ${firstScratcherConst};
}
                  ...
                  <div className="scratcher-editor" style={{ marginTop: '15px' }}>
                    ...
                    <div className="form-group">
                      ...
                      <input
                        ...
                        onChange={(e) => handleUpdateScratcher('scratchRadius', parseInt(e.target.value) || DEFAULT_SCRATCH_RADIUS)}
                        min="10"
                        max="100"
                      />
                    </div>
