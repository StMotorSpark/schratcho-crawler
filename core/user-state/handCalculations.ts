 ...
 * All values are rounded up to avoid decimal gold values.
 * 
...
export function calculateHandValue(
  ...
  // Initialize each ticket's calculated value with its base gold value (rounded)
  const ticketValues: number[] = calculatedTickets.map((t) => Math.ceil(t.goldValue));
  ...
  for (let i = 0; i < calculatedTickets.length; i++) {
    ...
    if (!effect) {
      // No effect - just use base gold value (rounded)
      ticket.calculation = {
        complete: true,
        calculatedValue: Math.ceil(ticket.goldValue),
      };
      continue;
    }
    ...
    // Apply changes to ticket values array (round up after modification)
...
function applyHandEffect(
  ...
  switch (operation) {
    case 'multiply':
      newValue = Math.ceil(currentValue * amount);
      break;
    case 'add':
      newValue = Math.ceil(currentValue + amount);
      break;
  ...
  if (targetIndex === HAND_TARGET_INDEX) {
    ...
    return {
      calculation: {
        complete: true,
        appliedEffect: effect,
        calculatedValue: Math.ceil(ticketValues[currentIndex]),
  ...
  return {
    calculation: {
      complete: true,
      appliedEffect: effect,
      calculatedValue: targetIndex === currentIndex ? Math.ceil(newValue) : Math.ceil(ticketValues[currentIndex]),
...
function applyDiffEffect(
  ...
  switch (appliedCondition.operation) {
    case 'multiply':
      newValue = Math.ceil(targetValue * appliedCondition.amount);
      break;
    case 'add':
      newValue = Math.ceil(targetValue + appliedCondition.amount);
      break;
    case 'subtract':
      ...
      break;
    case 'set':
      newValue = Math.ceil(appliedCondition.amount);
      break;
  }
