...
✅ **Load Existing Layouts**: Import and edit existing layouts from your core folder  
...
### Step 0: Load Existing Layout (Optional)

If you want to edit an existing ticket layout:

1. Click the **"📂 Load Existing Layout"** button
2. Navigate to `core/game-logic/tickets/` and select a `.ts` or `.json` file
3. The tool will load all layout data including areas, mechanics, and settings
4. You can now edit the layout and save it back

**Tip**: This is useful for making adjustments to existing tickets or creating variations.
...
Before exporting, test your ticket layout to ensure it works as expected:

1. Click **"▶ Show Test Panel"** in the right panel
2. The test panel allows you to:
   - **Simulate Scratching**: Click on areas to simulate revealing them
   - **Test Win Conditions**: See if your win condition works correctly
   - **Validate Mechanics**: Ensure reveal mechanics match your expectations

3. **Understanding Test Results**:
   - 🔒 = Area not yet revealed
   - ✓ = Area revealed
   - 🎉 WINNER! = Win condition met
   - ❌ Not a winner yet = More scratching needed

4. **Reset Test**: Click "🔄 Reset Test" to start over

**Best Practice**: Always test your layout before exporting to catch configuration issues early.
...
**This is the fastest way to integrate your layout:**

1. Click **"💾 Download for Core"** 
2. Save the file to `core/game-logic/tickets/[layoutId]Layout.ts`
3. The file includes:
   - Complete TypeScript code with proper imports
   - Version information and timestamp
   - Instructions for integration in the file header

**The generated file includes helpful comments:**

```typescript
/**
 * Treasure Map Ticket
 * A pirate-themed ticket with hidden treasure locations
 * 
 * Generated by Ticket Layout Designer
 * Date: 2025-11-24T15:45:06.175Z
 * 
 * To use this layout:
 * 1. Save this file to: core/game-logic/tickets/treasure-mapLayout.ts
 * 2. Import in core/mechanics/ticketLayouts.ts:
 *    import { TREASURE_MAP_TICKET } from '../game-logic/tickets/treasure-mapLayout';
 * 3. Add to TICKET_LAYOUTS object:
 *    'treasure-map': TREASURE_MAP_TICKET,
 */

import type { TicketLayout } from '../../mechanics/ticketLayouts';

export const TREASURE_MAP_TICKET: TicketLayout = {
  // ... configuration
};
```
...
The tool includes a built-in testing panel that allows you to simulate ticket behavior without running the full game:

#### How to Use the Test Panel

1. **Open the Test Panel**
   - Click "▶ Show Test Panel" in the right sidebar
   - The panel expands to show your scratch areas

2. **Simulate Scratching**
   - Click on any area button to toggle its "revealed" state
   - 🔒 = Hidden (not scratched)
   - ✓ = Revealed (scratched)

3. **Check Win Condition**
   - The panel automatically evaluates your win condition
   - Results appear immediately after each click:
     - 🎉 WINNER! = Win condition is satisfied
     - ❌ Not a winner yet = More areas need to be revealed

4. **View Details**
   - Bottom of test panel shows:
     - Current win condition type
     - Current reveal mechanic
     - Number of areas revealed

5. **Reset Test**
   - Click "🔄 Reset Test" to clear all revealed areas
   - Test different scenarios

#### Testing Different Win Conditions

**reveal-all-areas**
- All scratch areas must be revealed to win
- Test: Click all areas one by one
- Win condition met when all areas are revealed

**reveal-any-area**
- Win as soon as any single area is revealed
- Test: Click any area
- Win condition should trigger immediately

**match-symbols**
- Win when enough symbols match (match-two or match-three)
- Test: Reveal 2 or 3 areas (based on mechanic)
- Note: This is simplified for testing; actual game has random symbols

**progressive-reveal**
- Win when the last area is revealed
- Test: Click areas in order, ending with the last one
- Win condition only triggers on final area

#### Best Practices for Testing

1. **Test Edge Cases**
   - Try revealing areas in different orders
   - Test revealing just one area
   - Test revealing all areas

2. **Validate Win Conditions**
   - Ensure win condition matches your reveal mechanic
   - Test that wins trigger at the right time

3. **Check Area Count**
   - For match-three: Need at least 3 areas
   - For progressive: Need at least 2 areas
   - For reveal-all: Any number of areas works

4. **Document Expected Behavior**
   - Note any special win conditions in description
   - Document if certain areas are more important

### Version Tracking

The tool automatically adds version information to exported files:

- **Timestamp**: Date and time when layout was generated
- **Generation Info**: Notes that it was created by the Layout Designer
- **Integration Instructions**: Built-in documentation in file header

This helps you:
- Track when layouts were created or modified
- Understand the source of layout files
- Maintain multiple versions of the same layout

### Debugging Common Issues

**Issue: Win condition triggers too early**
- Check that winCondition matches revealMechanic
- Verify threshold percentages aren't too low
- Test with the interactive panel

**Issue: Win condition never triggers**
- Ensure all required areas can be revealed
- Check that win condition logic matches your intent
- Verify area IDs are correct and unique

**Issue: Layout looks different in game**
- Canvas dimensions affect mask quality, not display size
- Display size is controlled by width/heightPercent
...
✅ **Testing & Debugging Panel** (Added 2025-11-24)
- Interactive simulation of ticket scratching
- Real-time win condition validation
- Visual feedback for revealed areas

✅ **Load Existing Layouts** (Added 2025-11-24)
- Import .ts or .json layout files for editing
- Seamlessly update existing tickets
- Create variations of successful layouts

✅ **Core Integration** (Added 2025-11-24)
- Direct save to core game logic folder
- Auto-generated integration instructions
...
- [ ] Live preview with actual game rendering engine
